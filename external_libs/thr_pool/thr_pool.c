...
static int create_worker(thr_pool_t *pool)
{
    ...
    error = pthread_create(&thread_id, &pool->pool_attr, worker_thread, pool);
...
static void job_cleanup(thr_pool_t *pool)
{
    ...
    for (activepp = &pool->pool_active; (activep = *activepp) != NULL; activepp = &activep->active_next)
    {
        ...
        if(pthread_equal(activep->active_tid, my_tid))
        {
    ...
    for (;;)
    {
        ...
        while (pool->pool_head == NULL &&
            ...
            if (pool->pool_nthreads <= pool->pool_minimum) {
                ...
                (void) gettimeofday((struct timeval*)&ts, 0);
...
static void clone_attributes(pthread_attr_t *new_attr, pthread_attr_t *old_attr)
{
    ...
    if (old_attr != NULL)
    {
        ...
        pthread_attr_getstackaddr(old_attr,&addr);
        pthread_attr_getstacksize(old_attr,&size);
        pthread_attr_setstackaddr(new_attr,NULL);
