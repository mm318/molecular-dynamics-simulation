...
void analyze(OpenBox * const box, char * const run_name, int step, FILE *out)
{
    int i;
    double TE, Temp, vT_x, vT_y, vT_z, vT;
    ...
    if(step == 0) {
        ...
        fputs("t\tT.E.\tTemp.\tvT_x\tvT_y\tvT_z\tvT\n",out);
    }
    ...
    Temp = v_squared_sum/(3*box->num_particles);
    vT = sqrt(vT_x*vT_x + vT_y*vT_y + vT_z*vT_z);

    fprintf(out, "%f\t%f\t%f\t%.6g\t%.6g\t%.6g\t%.6g\n", step*TIMESTEP,
        TE, Temp, vT_x, vT_y, vT_z, vT);
...
int start_md(const int num_steps, const char output_png, char * const run_name)
{
    ...
    VisualContext *vc;  // data for opengl display
    ...
    s = (Seed) time(NULL);
    OpenBox *box = new_random(&s, 2*BOX_HALF_DIM, 2*BOX_HALF_DIM, 2*BOX_HALF_DIM);

    fprintf(stderr,"creating new visual\n");
    vc = new_visual(box);
    set_visual(vc);
    pthread_create(&vis_thr, NULL, start_visual, (void*) run_name);
